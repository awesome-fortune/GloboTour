package za.co.masekofortune.globotour.city
...
class CityAdapter(private val context: Context, var cityList: ArrayList<City>) :
    ...
    inner class CityViewHolder(itemView: View) :
        ...
        private fun addToFavorite() {
            currentCity?.isFavorite = !(currentCity?.isFavorite!!) // Toggle the 'isFavourite' Boolean value
            ...
            } else { // else it is not favorite - update and remove the city object from favorite list
                imvFavorite.setImageDrawable(icFavoriteBorderedImage)
                VacationSpots.favoriteCityList.remove(currentCity!!)
            }
        }
